...
from .constants import (
    UPLOAD_DIRECTORY,
    PROCESSED_DIRECTORY,
    HIGHLIGHTED_OUTPUT_DIRECTORY,
    OCRMYPDF_TEXT_DIRECTORY,
    LAYOUTPARSER_TEXT_DIRECTORY,
    CONFIG_PATH,
    WEIGHTS_PATH
)
...
def create_ocr_layered_pdf(original_path: str, processed_path: str) -> str:
    ...
    try:
        ...
        logging.info("PDF already contains text layer, creating copy...")
        os.replace(original_path, processed_path)
        return processed_path
    except Exception as e:
        logging.info(f"OCR processing failed: {str(e)}")
...
def debug_save_layoutparser_ocr_text(file_contents: bytes, filename: str):
    ...
    logging.info(f"Starting LayoutParser OCR debug for '{filename}'...")
    try:
        ...
        logging.info(f"Saved LayoutParser OCR text to: {output_path}")
    except Exception as e:
        logging.info(f"Error in LayoutParser debug: {str(e)}")
...
def process_and_embed_pdf(file_contents: bytes, filename: str):
    ...
    try:
        ...
        if not full_text.strip():
            logging.info(f"Warning: No text could be extracted from the ocrmypdf-processed file: {filename}")
            return
        ...
        logging.info(f"Successfully processed and embedded '{filename}'. Found {len(chunks)} chunks.")
    except Exception as e:
...
async def query_rag_pipeline(filename: str, question: str) -> Optional[Dict[str, str]]:
    ...
    try:
        ...
        logging.error(f"Error in RAG pipeline for '{filename}': {str(e)}")
...
def create_highlighted_pdf(original_filename: str, text_to_highlight: str, question_id: str = None) -> Optional[str]:
    ...
    try:
        ...
        for page in doc:
            ...
            for inst in text_instances:
                ...
                if (search_text in word_text or 
                    fuzz.ratio(search_text, word_text) > 85):
        ...
        logging.error(f"Error creating highlighted PDF for '{original_filename}': {str(e)}")
        return None